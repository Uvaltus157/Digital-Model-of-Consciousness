...
`src.apps.runner.UnifiedSystem` now defines its main hooks directly from the
...
RUNNER_PATCHES: List[RunnerPatchRecord] = [
    ...
    RunnerPatchRecord(
        target="UnifiedSystem.__init__",
        replacement="src.apps.runner_unified_init.initialize_unified_system",
        reason="system construction extracted from heavy runner",
    ),
    RunnerPatchRecord(
        target="UnifiedSystem.run",
        replacement="src.apps.runner_loop.run_unified_life_loop",
        reason="outer life loop extracted from heavy runner",
    ),
    RunnerPatchRecord(
        target="UnifiedSystem.resolve_module_training_flags_from_config",
    ...
    RunnerPatchRecord(
        target="UnifiedSystem._force_hover_flight_runtime_config",
    ...
    RunnerPatchRecord(
        target="UnifiedSystem.rebuild_optimizer_from_trainable_modules",
...
def apply_runner_patches(runner_runtime: Any, unified_system_cls: type) -> None:
    ...
    unified_system_cls.__init__ = initialize_unified_system
