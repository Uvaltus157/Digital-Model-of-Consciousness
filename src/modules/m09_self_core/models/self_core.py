...
class SelfCore(nn.Module):
    def __init__(self, cfg: Optional[SelfCoreConfig] = None):
        ...
        self.subjective_head = nn.Sequential(
            nn.Linear(self.cfg.self_dim + 6, self.cfg.hidden_dim),
            nn.SiLU(),
    ...
    def forward(
        ...
        device = next(self.parameters()).device
        cfg = self.cfg
        prev_state = prev_state or self.initial_state(1, device)
        ...
        x = torch.cat([body_state, action, tactile, vestibular, object_latent, workspace], dim=-1)
