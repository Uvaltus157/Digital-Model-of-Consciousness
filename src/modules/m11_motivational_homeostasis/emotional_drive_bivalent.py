...
class EmotionalDrive:
    ...
    def compute(self, out: Dict, obs: Optional[Dict] = None) -> Dict[str, torch.Tensor | float]:
        ...
        reflection = out.get("reflection_out", {}) if isinstance(out.get("reflection_out"), dict) else {}
        ...
        self_conf = _scalar(reflection.get("self_confidence"), 0.0)
        object_conf = _scalar(object_imagery.get("object_confidence"), 0.0)
        speech_conf = _scalar(symbolic.get("confidence"), 0.0)
        ...
        thought = out.get("thoughts", {}).get("thought") if isinstance(out.get("thoughts"), dict) else None
    ...
    def _zeros(self, out: Dict) -> Dict[str, torch.Tensor | float]:
        device = torch.device("cpu")
        for v in out.values():
            if isinstance(v, torch.Tensor):
                device = v.device
